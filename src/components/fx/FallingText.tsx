...
interface FallingTextProps {
  ...
  fontSize?: string;
  /** classes applied to every word span (e.g. pill styling) */
...
const FallingText: React.FC<FallingTextProps> = ({
  ...
  useEffect(() => {
    ...
    const newHTML = words
      .map((word) => {
        ...
        return `<span class="inline-block select-none ${wordClassName} ${
          isHighlighted ? "text-brand-magenta font-bold" : ""
        }">${word}</span>`;
      })
      .join(" ");

    textRef.current.innerHTML = newHTML;
  }, [text, highlightWords, wordClassName]);
  ...
  return (
    <div
      ref={containerRef}
      className="relative z-[1] h-full w-full cursor-grab overflow-hidden text-center"
